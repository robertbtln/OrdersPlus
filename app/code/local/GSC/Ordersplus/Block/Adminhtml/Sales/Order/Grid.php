    ...
    protected function _prepareCollection()
    {
        ...
        $collection->getSelect()->joinLeft(
            array('sfoi' => $resource->getTableName('sales_flat_order_item')),
                  'main_table.entity_id = sfoi.order_id AND sfoi.parent_item_id IS NULL',
                  array(
        ...
        $collection->getSelect()->joinLeft(
            array('sfog' => $resource->getTableName('sales_flat_order_grid')),
        ...
        $collection->getSelect()->joinLeft(
            array('sfoa' => $resource->getTableName('sales_flat_order_address')),
        ...
        $collection->getSelect()->joinLeft(
            array('sfop' => $resource->getTableName('sales_flat_order_payment')),
            'main_table.entity_id = sfop.parent_id',
            array('sfop.method')
        );

        // Comment Information
        // Note: Filtered for Parent Id, Visible on Frontend (true), Customer Notified (false), Status (null) [to work with total quatity]
        $collection->getSelect()->joinLeft(
            array('sfosh' => $resource->getTableName('sales_flat_order_status_history')),
            'main_table.entity_id = sfosh.parent_id AND sfosh.is_visible_on_front=true AND sfosh.is_customer_notified=false AND sfosh.status IS NULL',
            array('sfosh.comment')
        );

        // Group everything on main `entity_id`
